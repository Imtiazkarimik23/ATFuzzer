	...
	for line in run_command(command):
		...
		try:
			...
			if stime <= datetime_object<=ftime:
				if (line not in logcat) and ("/QC-time-services" not in line) and ("WakeLock( 2253)" not in line) and ("EarsSyncAdapter(10719)" not in line) and \
...
def fuzz_blue(cmd):
	...
	if r is None:
		retList.append(flag)
		return retList
	#if "ERROR" not in r: #cut phone Go to sleep got phone call
		#time.sleep(1)
		#send("AT+CHUP")
		#r10 = recv()
		#time.sleep(2)
		#print "man I am confused!!"
		#cmd2 = cmd + '\r\r'
...
def main():
	#fuzz_blue("D*")
	#fuzz_blue("+CSCS=GSM");
	#fuzz_blue("D;A;") #Interesting!!!!! #GOLD
	#fuzz_blue("DA;") #Interesting!!!!! #GOLD
	#fuzz_blue("DA;")  # Interesting!!!!! #GOLD
	'''
	flag =0
	for _ in range(30):
	...
	#fuzz_blue("D")
	#fuzz_blue("D@")
	#fuzz_blue("D**61*+1812555673*11*25#")
